...
#define TAG "ti.appinfo"

using namespace v8;

static Persistent<Object> bindingCache;

static void Versioncode_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Versioncode.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::VersioncodeBindings::lookupGeneratedInit(
...
static void Versioncode_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Versioncode_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Versioncode_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::VersioncodeBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry VersioncodeBinding = {
	"ti.appinfo",
	Versioncode_init,
	Versioncode_dispose
};
...
Java_ti_appinfo_VersioncodeBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.appinfo", &VersioncodeBinding);
	titanium::KrollBindings::addExternalLookup(&(::VersioncodeBindings::lookupGeneratedInit));
}
